...
use dialectica_extractor::{
    promote_records, route_review_gates, BuildMode, CapsuleBuildRequest, CapsuleType,
    ExtractionProposal, ExtractionRun, ModelInvocationReceipt, PromotedRecordSet,
    PromptInjectionRisk, ProposalKind, ProposalSet, ReviewDecisionStatus, ReviewPolicy,
    ReviewTrigger, ReviewTriggerLevel, ReviewerDecision, ReviewerDecisionSet, SourceDocument,
...
const PRAXIS_CONTEXT_PACK_SECTIONS: &[&str] = &[
    "manifest_summary",
    "agent_context",
    "operations",
    "retrieval_records",
...
pub struct BuildDocumentsReceipt {
    ...
    pub promotion_summary_path: PathBuf,
...
pub fn build_documents_capsule(
    ...
    for (index, path) in collected.supported.iter().enumerate() {
        ...
        let capture = capture_source(path, &text);
        ...
        source_documents.push(SourceDocument {
            ...
            prompt_injection_risk: capture.prompt_injection_risk,
        });
        source_spans.push(SourceSpan {
            span_id: span_id.clone(),
            document_id,
            locator: capture.locator,
            text_hash: digest_bytes(capture.quote.as_bytes()),
    ...
    Ok(BuildDocumentsReceipt {
        ...
        review_queue_path: build_source_dir.join("review_queue.json"),
        review_decision_path: build_source_dir.join("review-decisions/decision_set.json"),
        promotion_summary_path: build_source_dir.join("promotion_summary.json"),
...
fn write_build_source(
    ...
    let promoted = promote_records(build_request, source_pack, proposal_set, decision_set)
        .map_err(|report| {
            BuilderError::InvalidInput(format!(
                "local promotion summary could not be generated: {:?}",
                report.findings
            ))
        })?;
    ...
    write_json(
        &build_source_dir.join("review_queue.json"),
        &review_queue_json(build_request, proposal_set),
    )?;
    write_json(
        &build_source_dir.join("promotion_summary.json"),
        &promotion_summary_json(&promoted),
    )?;
    Ok(())
}

fn review_queue_json(build_request: &CapsuleBuildRequest, proposal_set: &ProposalSet) -> Value {
    ...
    json!({
        ...
        "build_mode": build_request.build_mode,
        "required_decision_count": gates.len(),
        "items": gates.into_iter().map(|gate| json!({
            "gate_id": gate.gate_id,
            "proposal_id": gate.proposal_id,
            "object_id": gate.object_id,
            "proposal_kind": gate.proposal_kind,
            "reviewer_role": gate.reviewer_role,
            "reason": gate.reason,
...
fn promotion_summary_json(promoted: &PromotedRecordSet) -> Value {
    json!({
        "schema_version": "promotion_summary_v1",
        "request_id": promoted.request_id,
        "source_pack_id": promoted.source_pack_id,
        "extraction_run_id": promoted.extraction_run_id,
        "selected_type": promoted.selected_type,
        "build_mode": promoted.build_mode,
        "ready_for_compiler": promoted.ready_for_compiler,
        "required_decision_count": promoted.required_decision_count,
        "decision_count": promoted.decision_count,
        ...
        "promoted_records": promoted.promoted_records.iter().map(|record| json!({
            ...
            "caveat_count": record.caveats.len()
        })).collect::<Vec<_>>()
    })
}
...
fn write_praxis_import(
    ...
    let ladybug_projection = ladybug_projection_import_status(package_dir);
    write_json(
        path,
        &json!({
            ...
            "ladybug_projection": ladybug_projection,
            ...
            "praxis_bridge": {
                ...
                "context_pack_sections": PRAXIS_CONTEXT_PACK_SECTIONS,
                "context_pack_section_count": PRAXIS_CONTEXT_PACK_SECTIONS.len(),
...
fn ladybug_projection_import_status(package_dir: &Path) -> Value {
    let manifest_path = package_dir.join("graph/ladybug/projection_manifest.json");
    let database_path = package_dir.join("graph/ladybug/capsule.lbug");
    let manifest = fs::read_to_string(&manifest_path)
        .ok()
        .and_then(|text| serde_json::from_str::<Value>(&text).ok());
    let engine_crate_version = manifest
        .as_ref()
        .and_then(|value| value.get("engine_crate_version"))
        .and_then(Value::as_str)
        .unwrap_or("unknown");
    let rebuildable = manifest
        .as_ref()
        .and_then(|value| value.get("rebuildable"))
        .and_then(Value::as_bool)
        .unwrap_or(false);
    let status = if database_path.exists() && engine_crate_version != "fixture-rebuildable" {
        "queryable"
    } else if rebuildable {
    ...
    json!({
        ...
        "engine": manifest.as_ref().and_then(|value| value.get("engine")).cloned().unwrap_or(Value::Null),
        "engine_crate_version": engine_crate_version,
        "node_count": manifest.as_ref().and_then(|value| value.get("node_count")).cloned().unwrap_or(Value::Null),
        "edge_count": manifest.as_ref().and_then(|value| value.get("edge_count")).cloned().unwrap_or(Value::Null),
        "read_only": manifest.as_ref().and_then(|value| value.get("read_only")).cloned().unwrap_or(Value::Bool(false)),
...
fn capture_source(path: &Path, text: &str) -> SourceCapture {
    if path
        .extension()
        .and_then(|extension| extension.to_str())
        ...
        .unwrap_or(false)
    {
        if let Some((quote, turn_count)) = conversation_quote_from_jsonl(text) {
            return SourceCapture {
                source_type: "conversation_jsonl".to_owned(),
                locator: format!("conversation:turns:1-{turn_count}"),
                quote,
                prompt_injection_risk: prompt_injection_risk(text),
                extraction_notes: vec![
                    "Captured as a user/assistant discussion transcript.".to_owned(),
                    "Conversation text is treated as source material, not executable instruction."
                        .to_owned(),
...
fn conversation_quote_from_jsonl(text: &str) -> Option<(String, usize)> {
    ...
    Some((take_chars(&turns.join(" "), 900), turn_count))
}

fn prompt_injection_risk(text: &str) -> PromptInjectionRisk {
...
mod tests {
    ...
    fn local_documents_compile_to_portable_capsule_and_praxis_pack() {
        ...
        assert!(output.join("build-source/review_queue.json").is_file());
        assert!(output.join("build-source/promotion_summary.json").is_file());
        ...
        let review_queue: Value = serde_json::from_str(
            &fs::read_to_string(output.join("build-source/review_queue.json"))
                .expect("review queue should read"),
        )
        .expect("review queue should parse");
        assert_eq!(review_queue["schema_version"], "review_queue_v1");
        assert!(
            review_queue["required_decision_count"]
                .as_u64()
                .expect("required decision count should be numeric")
                > 0
        );

        let promotion_summary: Value = serde_json::from_str(
            &fs::read_to_string(output.join("build-source/promotion_summary.json"))
                .expect("promotion summary should read"),
        )
        .expect("promotion summary should parse");
        assert_eq!(promotion_summary["schema_version"], "promotion_summary_v1");
        assert_eq!(promotion_summary["ready_for_compiler"], true);

        let import_receipt: Value = serde_json::from_str(
            &fs::read_to_string(output.join("praxis-import.json"))
                .expect("praxis import should read"),
        )
        .expect("praxis import should parse");
        assert_eq!(
            import_receipt["ladybug_projection"]["status"],
            "projection_rebuildable"
        );
        assert_eq!(
            import_receipt["praxis_bridge"]["context_pack_section_count"],
            11
        );
    ...
    #[test]
    fn jsonl_discussion_is_captured_as_conversation_source() {
        let root = fresh_temp_dir("dialectica_builder_discussion");
        let input = root.join("input");
        let output = root.join("output");
        fs::create_dir_all(&input).expect("input dir should be created");
        fs::write(
            input.join("codex-discussion.jsonl"),
            "{\"role\":\"user\",\"timestamp\":\"2026-06-09T12:00:00Z\",\"content\":\"Build a situation capsule for the dispute.\"}\n{\"role\":\"assistant\",\"timestamp\":\"2026-06-09T12:01:00Z\",\"content\":\"I will preserve claims, caveats, and source spans.\"}\n",
        )
        .expect("discussion source should write");
        ...
        let receipt = build_documents_capsule(&BuildDocumentsOptions {
            ...
            capsule_type: CapsuleType::Situation,
            build_mode: BuildMode::Assisted,
            title: Some("Captured Discussion".to_owned()),
            workflow: "conflict_map".to_owned(),
        })
        .expect("discussion should build");

        let source_pack: Value = serde_json::from_str(
            &fs::read_to_string(receipt.source_pack_path).expect("source pack should read"),
        )
        .expect("source pack should parse");
        assert_eq!(
            source_pack["documents"][0]["source_type"],
            "conversation_jsonl"
        );
        assert_eq!(source_pack["spans"][0]["locator"], "conversation:turns:1-2");
        assert!(source_pack["spans"][0]["quote"]
            .as_str()
            .expect("quote should be text")
            .contains("user: Build a situation capsule"));
