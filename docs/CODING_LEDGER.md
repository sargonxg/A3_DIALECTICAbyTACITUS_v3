...
- fixture PRAXIS contract JSON export produces the import shape consumed by
...
cargo run -p dialectica-cli -- verify $env:TEMP\dialectica-golden-v3
...
cargo run -p dialectica-cli -- praxis-export $env:TEMP\dialectica-golden-v3 --out $env:TEMP\dialectica-golden-v3-praxis-export.json
...
7. deepen v3 validation across claims, sources, graph, review, reasoning, and
   runtime records;
8. add store-backed build job state and repository interfaces;
9. add PostgreSQL migrations for capsule build state and artifacts;
10. add task-handler routes after store-backed jobs exist;
