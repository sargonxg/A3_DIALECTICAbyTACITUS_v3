...
| Source | Fresh finding | DIALECTICA consequence |
| --- | --- | --- |
| MCP transports spec, <https://modelcontextprotocol.io/specification/2025-11-25/basic/transports> | The current stdio transport uses UTF-8 JSON-RPC messages over stdin/stdout, delimited by newlines, and forbids non-MCP stdout. | The local `dialectica-mcp` server uses newline-delimited JSON-RPC and keeps logs on stderr. |
| MCP tools spec, <https://modelcontextprotocol.io/specification/2025-11-25/server/tools> | MCP tools are model-controlled, declared through `tools/list`, called through `tools/call`, and should keep human visibility/confirmation in the loop. | Expose local build tools for Codex, but keep their outputs caveated and preserve human review as explicit review decisions. |
| MCP resources spec, <https://modelcontextprotocol.io/specification/2025-11-25/server/resources> | MCP resources expose URI-addressed context to clients. | Expose welcome, builder contract, and PRAXIS bridge notes as `dialectica://` resources. |
